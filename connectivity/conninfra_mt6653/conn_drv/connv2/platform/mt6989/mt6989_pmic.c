...
static int consys_plt_pmic_gps_power_ctl_rc_mode(unsigned int enable)
{
	...
	if (enable) {
		 ...
		 * 2. set PMIC VRFIO18 LDO SW_OP_EN =1, SW_EN = 0, SW_LP =0 (sw disable)
		 */
...
static int consys_plt_pmic_gps_power_ctl_legacy_mode(unsigned int enable)
{
	...
	if (enable) {
		...
		if (ret)
			pr_notice("[%s] Enable VRFIO18 fail. ret=%d\n", __func__, ret);
	} else {
		/* VCN18 (MT6363 workaround VRFIO18 -> VCN18) */
		...
		if (ret)
			pr_notice("[%s] Disable VRFIO18 fail. ret=%d", __func__, ret);
	}

	return 0;
}

int consys_plt_pmic_gps_power_ctrl_mt6989(unsigned int enable)
{
