...
static bool g_is_vsel_enable = false;
...
int connv3_plt_pmic_initial_setting_mt6989v2(struct platform_device *pdev, struct connv3_dev_cb* dev_cb);
int connv3_plt_pmic_common_power_ctrl_mt6989v2(u32 enable);
int connv3_plt_pmic_vsel_ctrl_mt6989v2(u32 enable);
int connv3_plt_pmic_parse_state_mt6989v2(char *buffer, int buf_sz);
int connv3_plt_pmic_get_connsys_chip_info_mt6989v2(char *connsys_ecid, int connsys_ecid_size);
int connv3_plt_pmic_get_connsys_adie_chip_info_mt6989v2(char *connsys_adie_chip_info, int connsys_adie_chip_info_size);
int connv3_plt_pmic_get_pmic_chip_info_mt6989v2(char *pmic_ecid, int pmic_ecid_size);
int connv3_plt_pmic_pwr_rst_mt6989v2(void);

const struct connv3_platform_pmic_ops g_connv3_platform_pmic_ops_mt6989v2 = {
	.pmic_initial_setting = connv3_plt_pmic_initial_setting_mt6989v2,
	.pmic_common_power_ctrl = connv3_plt_pmic_common_power_ctrl_mt6989v2,
	.pmic_vsel_ctrl = connv3_plt_pmic_vsel_ctrl_mt6989v2,
	.pmic_parse_state = connv3_plt_pmic_parse_state_mt6989v2,
	.pmic_get_connsys_chip_info = connv3_plt_pmic_get_connsys_chip_info_mt6989v2,
	.pmic_get_connsys_adie_chip_info = connv3_plt_pmic_get_connsys_adie_chip_info_mt6989v2,
	.pmic_get_pmic_chip_info = connv3_plt_pmic_get_pmic_chip_info_mt6989v2,
	.pmic_pwr_rst = connv3_plt_pmic_pwr_rst_mt6989v2,
};

struct work_struct g_pmic_faultb_work_mt6989v2;
unsigned int g_spurious_pmic_exception_mt6989v2 = 1;
int g_faultb_gpio_mt6989v2 = -1, g_pmic_en_gpio_mt6989v2 = -1;

static irqreturn_t pmic_fault_handler(int irq, void * arg)
{
	if (g_spurious_pmic_exception_mt6989v2) {
		pr_info("[%s] g_spurious_pmic_exception_mt6989v2\n", __func__);
		return IRQ_HANDLED;
	}

	pr_notice("[%s] Get PMIC FaultB interrupt\n", __func__);
	schedule_work(&g_pmic_faultb_work_mt6989v2);
...
static void check_faultb_status(struct work_struct *work)
{
	...
	if ((g_faultb_gpio_mt6989v2 != -1) && (g_pmic_en_gpio_mt6989v2 != -1)) {
		faultb_level = gpio_get_value(g_faultb_gpio_mt6989v2);
		pr_info("[%s] PMIC_EN=%d, faultb=%d\n",
			__func__, gpio_get_value(g_pmic_en_gpio_mt6989v2), faultb_level);
		if (faultb_level == 1)
			return;
	}
...
int connv3_plt_pmic_common_power_ctrl_mt6989v2(u32 enable)
{
	...
	if (enable == 1 && pmic_enabled == 0) {
		...
		g_spurious_pmic_exception_mt6989v2 = 0;
		pmic_enabled = 1;
		...
		g_spurious_pmic_exception_mt6989v2 = 1;
...
int connv3_plt_pmic_vsel_ctrl_mt6989v2(u32 enable)
{
	...
	if (!g_is_vsel_enable) {
		pr_info("[%s] vsel not enable(%d)\n", __func__, g_is_vsel_enable);
		return 0;
	}
	...
	if (enable) {
		...
		if (!IS_ERR(vsel_set)) {
			...
			if (ret)
				pr_notice("[%s] vsel low fail, %d", __func__, ret);
		} else {
			pr_notice("[%s] fail to get \"connsys-pin-pmic-vsel-low\"", __func__);
		}
	} else {
		vsel_set = pinctrl_lookup_state(
		...
		if (!IS_ERR(vsel_set)) {
			ret = pinctrl_select_state(g_pinctrl_ptr, vsel_set);
			if (ret)
				pr_notice("[%s] vsel high fail, %d", __func__, ret);
		} else {
			pr_notice("[%s] fail to get \"connsys-pin-pmic-vsel-high\"", __func__);
		}
	}
...
int connv3_plt_pmic_initial_setting_mt6989v2(
	struct platform_device *pdev, struct connv3_dev_cb* dev_cb)
{
	...
	/* check vsel enable or not from dts */
	ret = of_property_read_u32(pdev->dev.of_node, "vsel-enable", &value);
	if (ret)
		pr_notice("[%s] read co_clock prop fail\n", __func__);
	else
		g_is_vsel_enable = (bool)value;
	pr_info("[%s] g_is_vsel_enable=%d\n", __func__, value);

	INIT_WORK(&g_pmic_faultb_work_mt6989v2, check_faultb_status);
	g_faultb_gpio_mt6989v2 = of_get_named_gpio(pdev->dev.of_node, "mt6376-gpio", 0);
	g_pmic_en_gpio_mt6989v2 = of_get_named_gpio(pdev->dev.of_node, "mt6376-gpio", 1);
