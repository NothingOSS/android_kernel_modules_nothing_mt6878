...
static int btmtk_post_chip_rst_on_handler(unsigned int type)
{
	...
	u8 default_baudrate_cmd[] = { 0x01, 0x6F, 0xFC, 0x0A, 0x01, 0x04, 0x06, 0x00, 0x01, 0x01, 0xC2, 0x01, 0x00, 0x03 };
	u8 default_baudrate_evt[] = {0x04, 0xE4, 0x06, 0x02, 0x04, 0x02, 0x00, 0x00, 0x01};
	u8 wakeup_cmd[] = { 0xFF };
	...
	if (g_sbdev->reset_type == CONNV3_CHIP_RST_TYPE_DFD_DUMP) {
		...
		ret = btmtk_main_send_cmd(g_sbdev, default_baudrate_cmd, sizeof(default_baudrate_cmd), default_baudrate_evt, sizeof(default_baudrate_evt), DELAY_TIMES, RETRY_TIMES, BTMTK_TX_PKT_SEND_NO_ASSERT, CMD_NO_NEED_FILTER);
		BTMTK_INFO("%s, set back default baudrate 115200, ret = %d", __func__, ret);
		if (ret < 0)
			BTMTK_ERR("%s: set back default baudrate failed, ret[%d]", __func__, ret);

		ret = btmtk_main_send_cmd(g_sbdev, wakeup_cmd, 1, NULL, 0, DELAY_TIMES, RETRY_TIMES, BTMTK_TX_PKT_SEND_DIRECT_NO_ASSERT, CMD_NO_NEED_FILTER);
		if (ret < 0)
			BTMTK_ERR("%s: wakeup_cmd failed, ret[%d]", __func__, ret);
	}
...
void btmtk_uart_wakeup_cb(void *para)
{
	...
	if (bmain_info->hif_hook.wakeup_host)
		...
		BTMTK_WARN("%s: bmain_info->hif_hook.wakeup_host is NULL", __func__);
}
...
int btmtk_connv3_sub_drv_init(struct btmtk_dev *bdev)
{
	...
	tty->dev->of_node = of_find_compatible_node(NULL, NULL, "mediatek,mt6989v2-conn_scp");
	if (!tty->dev->of_node)
		BTMTK_ERR("[ERR] %s: mediatek,mt6989v2-conn_scp of_node not found", __func__);

	ret = of_property_read_u64(tty->dev->of_node, "dfd-value-addr", &bdev->dfd_value_addr);
	if (ret < 0)
		BTMTK_ERR("[ERR] %s: mediatek,mt6989v2-conn_scp dfd-value-addr ret[%d]", __func__, ret);
	...
	if (ret < 0)
		BTMTK_ERR("[ERR] %s: mediatek,mt6989v2-conn_scp dfd-value-size ret[%d]", __func__, ret);
...
void btmtk_fwlog_flow_ctrl_work(struct work_struct *work)
{
	int ret = 0;
	struct btmtk_main_info *bmain_info = btmtk_get_main_info();
	//struct btmtk_dev *bdev = container_of(work, struct btmtk_dev, fwlog_flow_ctrl_work);
	struct btmtk_uart_dev *cif_dev = NULL;
	u8 *cmd = NULL;
	int cmd_len = 0;
	...
	if (btmtk_fops_get_state(g_sbdev) != BTMTK_FOPS_STATE_OPENED) {
		BTMTK_INFO("%s: bt already closed", __func__);
		return;
	}
	...
	if (cif_dev->own_state != BTMTK_DRV_OWN && cif_dev->own_state != BTMTK_DRV_OWNING) {
		BTMTK_INFO("%s: not in driver own[%d], skip", __func__, cif_dev->own_state);
		return;
	}

	mutex_lock(&bmain_info->fwlog_ctrl_lock);

	/* payload + cmd header */
	cmd_len = bmain_info->fwlog_flow_ctrl_data_len + PATCH_HCI_HEADER_SIZE;

	/* allocate and format anchor cmd */
	cmd = kmalloc(cmd_len, GFP_KERNEL);
	if (!cmd) {
		BTMTK_ERR("%s: alloc memory size(%d) failed", __func__, cmd_len);
		mutex_unlock(&bmain_info->fwlog_ctrl_lock);
		return;
	}
	...
	cmd[1] = 0x6C;
	cmd[2] = 0xFC;
	cmd[3] = bmain_info->fwlog_flow_ctrl_data_len;

	/* copy payload */
	memcpy(&cmd[PATCH_HCI_HEADER_SIZE], &bmain_info->fwlog_flow_ctrl_data[0],
			bmain_info->fwlog_flow_ctrl_data_len);

	mutex_unlock(&bmain_info->fwlog_ctrl_lock);

	BTMTK_DBG_RAW(cmd, cmd_len, "%s: len[%d] cmd:", __func__, cmd_len);
