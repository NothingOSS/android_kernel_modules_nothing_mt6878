...
static int btmtk_uart_send_xo_param_cmd(struct hci_dev *hdev, CONNXO_CFG_PARAM_STRUCT *connxo_cfg_param)
{
	...
	BTMTK_INFO("%s: connv3_get_plat_config: 0x%02x, 0x%02x, 0x%02x, 0x%02x", __func__, data[0], data[1], data[2], data[3]);
...
static int btmtk_uart_fw_own(struct btmtk_dev *bdev)
{
	...
	flush_work(&bmain_info->fwlog_flow_ctrl_work);
	...
	if (event_compare_status == BTMTK_EVENT_COMPARE_STATE_NEED_COMPARE ||
			!skb_queue_empty(&cif_dev->tx_queue)) {
		BTMTK_WARN("%s: during send_and_recv[%d] or tx queue have data[%d], keep drv own", __func__, event_compare_status, !skb_queue_empty(&cif_dev->tx_queue));
		cif_dev->own_state = BTMTK_DRV_OWN;
...
int btmtk_met_log_handler(struct btmtk_dev *bdev, u8 *buf, u32 size)
{
	...
	for (i = 0; i <= size - single_data_len; i += single_data_len) {
		if (single_data_len == 4)
			...
			len = snprintf(raw_buf + strlen(raw_buf), MET_LOG_MAX_BUF_SIZE - strlen(raw_buf),
						"%02X%02X%02X%02X%02X%02X%02X%02X,", buf[i+7], buf[i+6], buf[i+5], buf[i+4],
