...
static int xo_read_nvram(CONNXO_CFG_PARAM_STRUCT *pConnxo_cfg_param)
{
    ...
    if (!hcustlib) {
        BPRINT_E("%s(), dlopen fail! (%s)", __FUNCTION__, dlerror());
        return -1;
    }

    unsigned int* iFileXOILID = (unsigned int*)(dlsym(hcustlib, "iFileCONNXOLID")); /* AP_CFG_RDEB_FILE_CONNXO_LID */
    if (!iFileXOILID) {
        BPRINT_E("can not get iFileCONNXOLID from libcustom_nvram.so");
        dlclose(hcustlib);
        return -1;
    }
    ...
    BPRINT_I("%s: iFileXOILID[%u] xo_nvram_fd[%d] rec_size[%d] rec_num[%d]",
            __func__, *iFileXOILID, xo_nvram_fd.iFileDesc, rec_size, rec_num);

    if (xo_nvram_fd.iFileDesc < 0) {
        BPRINT_E("%s: xo_nvram_fd.iFileDesc < 0", __func__);
        dlclose(hcustlib);
        return -1;
    }
