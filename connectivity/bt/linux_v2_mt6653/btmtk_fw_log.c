...
coredump_fail_unlock:
		...
		skb_pull(skb, 4);
		fwlog_count++;
		BTMTK_INFO_LIMITTED("fw log counter[%d], log_level[%d], store in fw log", fwlog_count, bmain_info->fw_log_on);
		...
		BTMTK_DBG_RAW(skb->data, skb->len, "%s: fwlog_flow_ctrl evt len[%d] %02x ",
				__func__, skb->len + 1, hci_skb_pkt_type(skb));

		mutex_lock(&bmain_info->fwlog_ctrl_lock);
		/* parse payload length */
		bmain_info->fwlog_flow_ctrl_data_len = skb->data[1];

		/* error handle */
		if (bmain_info->fwlog_flow_ctrl_data_len > FWLOG_FLOW_CTRL_DATA_MAX_LEN) {
			BTMTK_WARN("%s: fwlog_flow_ctrl_data_len(%d) more than %d, skip",
					__func__, bmain_info->fwlog_flow_ctrl_data_len, FWLOG_FLOW_CTRL_DATA_MAX_LEN);
			mutex_unlock(&bmain_info->fwlog_ctrl_lock);
			return 1;
		}

		/* record packet data and send ack cmd to fw */
		memcpy(&bmain_info->fwlog_flow_ctrl_data[0], &skb->data[2],
				bmain_info->fwlog_flow_ctrl_data_len);
		mutex_unlock(&bmain_info->fwlog_ctrl_lock);

		/* ack cmd to fw */
		ret = schedule_work(&bmain_info->fwlog_flow_ctrl_work);

		if (!ret)
			BTMTK_DBG("%s: schedule_work already in the queue or executing", __func__);

		return 1;
	} else if (bt_cb(skb)->pkt_type == HCI_EVENT_PKT && skb->data[0] == 0xE3) {
		BTMTK_INFO_RAW(skb->data, skb->len, "%s: get E3 event and drop it, len[%d] %02x",
				__func__, skb->len + 1, hci_skb_pkt_type(skb));
		return 1;
	}
