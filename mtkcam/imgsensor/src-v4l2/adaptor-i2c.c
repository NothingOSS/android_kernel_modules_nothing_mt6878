		...
		u16 addr, u8 reg, u8 *val)
{
	...
	if (ret < 0) {
		dev_info(&i2c_client->dev, "rd_u8_u8:addr = %x, reg = %x, i2c transfer failed (%d)\n", addr, reg, ret);
		return ret;
	}
	dev_info(&i2c_client->dev, "out rd_u8_u8:addr = %x, reg = %x, i2c transfer = (%x)\n", addr, reg, ret);
		...
		u16 addr, u8 reg, u8 val)
{
	...
	if (ret < 0)
		dev_info(&i2c_client->dev, "wr_u8_u8:addr = %x, reg = %x, i2c transfer failed (%d)\n", addr, reg, ret);
		...
		u16 addr, u16 *list, u32 len)
{
	...
	while (sent < total) {
		...
		if (ret != cnt) {
			...
			"wr_regs_u8_u8:addr = %x,, GC02M1 i2c transfer failed (%d)\n", addr, ret);
			kfree(pmem);
			return -EIO;
		}
