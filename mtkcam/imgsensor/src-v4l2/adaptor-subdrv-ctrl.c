	...
	if (ctx->extend_frame_length_en == FALSE) {
		if (addr_ll) {
			...
			if(ctx->exposure[0] >= (ctx->s_ctx.frame_length_max - ctx->s_ctx.exposure_margin) &&
			   (ctx->s_ctx.sensor_id == S5KGN9SP_SENSOR_ID || ctx->s_ctx.sensor_id == S5KGN9SPOFXIAN_SENSOR_ID)) {
...
void set_long_exposure(struct subdrv_ctx *ctx)
{
	...
	if (shutter > (ctx->s_ctx.frame_length_max - ctx->s_ctx.exposure_margin)) {
		...
		if(ctx->s_ctx.sensor_id == S5KGN9SP_SENSOR_ID || ctx->s_ctx.sensor_id == S5KGN9SPOFXIAN_SENSOR_ID) {
...
void streaming_control(struct subdrv_ctx *ctx, bool enable)
{
	...
	if (enable) {
		if(ctx->s_ctx.sensor_id == GC08A8_SENSOR_ID || ctx->s_ctx.sensor_id == GC08A8XL_SENSOR_ID || ctx->s_ctx.sensor_id == GC08A8SYX_SENSOR_ID){
