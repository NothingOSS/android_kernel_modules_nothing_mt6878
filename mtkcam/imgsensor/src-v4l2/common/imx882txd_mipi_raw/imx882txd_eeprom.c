...
static DEFINE_MUTEX(gimx882txd_eeprom_mutex);
...
static bool read_imx882txd_eeprom(struct subdrv_ctx *ctx,
...
static struct EEPROM_PDAF_INFO *get_eeprom_pdaf_info(struct subdrv_ctx *ctx)
{
	...
	mutex_lock(&gimx882txd_eeprom_mutex);
	if (pinfo == NULL) {
		read_imx882txd_eeprom(ctx, 0x1, read_data, 4);
		if (((read_data[3] << 24) |
		     (read_data[2] << 16) |
		     (read_data[1] << 8) |
	...
	mutex_unlock(&gimx882txd_eeprom_mutex);

	return pinfo;
}

unsigned int read_imx882txd_LRC(struct subdrv_ctx *ctx, BYTE *data)
{
	...
	LOG_INF("read imx882txd LRC, addr = %d, size = %u\n",
		pinfo->LRC_addr, pinfo->LRC_size);

	mutex_lock(&gimx882txd_eeprom_mutex);
	if ((readed_size == 0) &&
	    read_imx882txd_eeprom(ctx, pinfo->LRC_addr,
...
unsigned int read_imx882txd_DCC(struct subdrv_ctx *ctx, BYTE *data)
{
	static BYTE IMX519_DCC_data[DCC_SIZE] = { 0 };
	static unsigned int readed_size;
	struct EEPROM_PDAF_INFO *pinfo = get_eeprom_pdaf_info(ctx);

	LOG_INF("read imx882txd DCC, addr = %d, size = %u\n",
		pinfo->DCC_addr, pinfo->DCC_size);

	mutex_lock(&gimx882txd_eeprom_mutex);
	if ((readed_size == 0) &&
	    read_imx882txd_eeprom(ctx, pinfo->DCC_addr,
