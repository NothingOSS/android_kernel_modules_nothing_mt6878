 ...
 *============================================================================
 ****************************************************************************/
#include "gc08a8xl_mipiraw_Sensor.h"

#define LOG_TAG "[gc08a8xl]"
#define LOG_INF(format, args...) pr_info(LOG_TAG "[%s] " format, __func__, ##args)

static int gc08a8xl_set_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int gc08a8xl_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int gc08a8xl_set_shutter_frame_length(struct subdrv_ctx *ctx,u8 *para, u32 *len);
static int gc08a8xl_set_multi_shutter_frame_length(struct subdrv_ctx *ctx,u8 *para, u32 *len);
static int gc08a8xl_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int gc08a8xl_set_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int gc08a8xl_set_max_framerate_by_scenario(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int init_ctx(struct subdrv_ctx *ctx,	struct i2c_client *i2c_client, u8 i2c_write_id);
static int gc08a8xl_get_imgsensor_id(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int get_imgsensor_id(struct subdrv_ctx *ctx, u32 *para);
static int gc08a8xl_open(struct subdrv_ctx *ctx);

/* STRUCT */

static struct subdrv_feature_control feature_control_list[] = {
	{SENSOR_FEATURE_SET_TEST_PATTERN, gc08a8xl_set_test_pattern},
	{SENSOR_FEATURE_SET_GAIN, gc08a8xl_set_gain},
	{SENSOR_FEATURE_SET_ESHUTTER, gc08a8xl_set_shutter},
	{SENSOR_FEATURE_SET_SHUTTER_FRAME_TIME, gc08a8xl_set_shutter_frame_length},
	{SENSOR_FEATURE_SET_MULTI_SHUTTER_FRAME_TIME,gc08a8xl_set_multi_shutter_frame_length},
	{SENSOR_FEATURE_SET_FRAMELENGTH, gc08a8xl_set_frame_length},
	{SENSOR_FEATURE_SET_MAX_FRAME_RATE_BY_SCENARIO, gc08a8xl_set_max_framerate_by_scenario},
	{SENSOR_FEATURE_CHECK_SENSOR_ID, gc08a8xl_get_imgsensor_id},
};

// 1000 base for dcg gain ratio
//static u32 gc08a8xl_dag_ratio_table_12bit[] = {4000};
...
static struct subdrv_mode_struct mode_struct[] = {
	{
		...
		.mode_setting_table = gc08a8xl_3264x2448_30fps_addr_data,
		.mode_setting_len = ARRAY_SIZE(gc08a8xl_3264x2448_30fps_addr_data),
	...
	},
	{
		...
		.mode_setting_table = gc08a8xl_3264x1836_30fps_addr_data,
		.mode_setting_len = ARRAY_SIZE(gc08a8xl_3264x1836_30fps_addr_data),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	// 		.dcg_gain_table = gc08a8xl_dag_ratio_table_12bit,
	// 		.dcg_gain_table_size = sizeof(gc08a8xl_dag_ratio_table_12bit),
	// 	},
	// },
};

static struct subdrv_static_ctx static_ctx = {
	.sensor_id = GC08A8XL_SENSOR_ID,
	...
	.ana_gain_type = 4,
	.ana_gain_step = 1,
	.ana_gain_table = gc08a8xl_ana_gain_table,
	.ana_gain_table_size = sizeof(gc08a8xl_ana_gain_table),
	.min_gain_iso = 100,
	.exposure_def = 0x3D0,
	.exposure_min = 4,
	...
	.init_setting_table = gc08a8xl_init_setting,
	.init_setting_len = ARRAY_SIZE(gc08a8xl_init_setting),
	.mode = mode_struct,
...
const struct subdrv_entry gc08a8xl_mipi_raw_entry = {
	.name = "gc08a8xl_mipi_raw",
	.id = 	GC08A8XL_SENSOR_ID,
	.pw_seq = pw_seq,
...
static int gc08a8xl_get_imgsensor_id(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	while (ctx->s_ctx.i2c_addr_table[i] != 0xFF) {
		...
		do {
			...
			LOG_INF("[gc08a8xl]: i2c write id: 0x%x, sensor id: 0x%x, module id: 0x%x\n",
			...
			LOG_INF("[gc08a8xl]: Read sensor id fail! i2c write id: 0x%x, sensor id: 0x%x\n",
...
void gc08a8xl_set_dummy(struct subdrv_ctx *ctx)
{
...
static void gc08a8xl_set_max_framerate(struct subdrv_ctx *ctx, UINT16 framerate, kal_bool min_framelength_en)
{
...
static int gc08a8xl_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	return gc08a8xl_set_shutter_frame_length(ctx, para, len);
}
	...
	if (shutter_cnt == 1) {
		...
		if (ctx->autoflicker_en) {
			...
			if (realtime_fps >= 593 && realtime_fps <= 607)
				gc08a8xl_set_max_framerate(ctx, 592, 0);
			else if (realtime_fps >= 297 && realtime_fps <= 305)
				gc08a8xl_set_max_framerate(ctx, 296, 0);
			else if (realtime_fps >= 147 && realtime_fps <= 150)
				gc08a8xl_set_max_framerate(ctx, 146, 0);
		}
