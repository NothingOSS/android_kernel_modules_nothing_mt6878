 ...
 *============================================================================
 ****************************************************************************/
#define PFX "sc202cssj_camera_sensor"
...
#include "sc202cssj_mipi_raw_Sensor.h"
#include "sc202cssj_ana_gain_table.h"
...
#define sc202cssj_table_write_cmos_sensor(...) subdrv_i2c_wr_regs_u8_u8(__VA_ARGS__)

#define LOG_INF(format, args...) pr_err(PFX "[%s] " format, __func__, ##args)
#define SC202CSSJ_LOG_DBG(format, args...) pr_err(PFX "[%s] " format, __func__, ##args)

#define FPT_PDAF_SUPPORT 0
#define SC202CSSJ_SENSOR_GAIN_MAX_VALID_INDEX  5
#define SC202CSSJ_SENSOR_GAIN_MAP_SIZE         5
#define SC202CSSJ_SENSOR_BASE_GAIN           0x400
#define SC202CSSJ_SENSOR_MAX_GAIN            (16 * SC202CSSJ_SENSOR_BASE_GAIN)

#define EEPROM_SC202CSSJ_MOLDULEID 0x35
#define EEPROM_SC202CSSJ_MOLDULEID_0_ADDR 0x8009
#define EEPROM_SC202CSSJ_MOLDULEID_1_ADDR 0x8016
...
static void set_frame_length(struct subdrv_ctx *ctx, kal_uint16 frame_length)
{
	...
	SC202CSSJ_LOG_DBG("Framelength: set=%d/input=%d/min=%d\n",
				     ...
				     kal_bool auto_extend_en)
{
	...
	write_cmos_sensor_8(ctx, 0x3e01, (shutter >> 4) & 0xFF);
	write_cmos_sensor_8(ctx, 0x3e02, (shutter << 4) & 0xF0);
	SC202CSSJ_LOG_DBG("set shutter =%d, framelength =%d\n", shutter, ctx->frame_length);
				...
				kal_uint16 frame_length)
{
	...
	SC202CSSJ_LOG_DBG("shutter_cnt = %d, set shutter =%d, framelength =%d, vrealtime_fps = %d, ctx->autoflicker_en = %d\n",
...
static kal_uint16 gain2reg(struct subdrv_ctx *ctx, const kal_uint32 gain)
{
	...
	if (reg_gain < SC202CSSJ_SENSOR_BASE_GAIN)
		reg_gain = SC202CSSJ_SENSOR_BASE_GAIN;
	else if (reg_gain > SC202CSSJ_SENSOR_MAX_GAIN)
		reg_gain = SC202CSSJ_SENSOR_MAX_GAIN;
...
static kal_uint32 set_gain(struct subdrv_ctx *ctx, kal_uint32 gain)
{
	...
	kal_uint16 SC202CSSJ_AGC_Param[SC202CSSJ_SENSOR_GAIN_MAP_SIZE][2] = {
		{  1024,  0x00 },
		{  2048,  0x01 },
		{  4096,  0x03 },
		{  8192,  0x07 },
	...
	for (gain_index = SC202CSSJ_SENSOR_GAIN_MAX_VALID_INDEX - 1; gain_index >= 0; gain_index--)
		if (reg_gain >= SC202CSSJ_AGC_Param[gain_index][0])
			break;

	write_cmos_sensor_8(ctx, 0x3e09, SC202CSSJ_AGC_Param[gain_index][1]);
	temp_gain = reg_gain * SC202CSSJ_SENSOR_BASE_GAIN / SC202CSSJ_AGC_Param[gain_index][0];
	write_cmos_sensor_8(ctx, 0x3e07, (temp_gain >> 3) & 0xff);
	LOG_INF("SC202CSSJ_AGC_Param[gain_index][1] = 0x%x, temp_gain = 0x%x, reg_gain = %d, gain = %d\n",
		SC202CSSJ_AGC_Param[gain_index][1], temp_gain, reg_gain, gain);
	return reg_gain;
}
...
/*static kal_uint32 return_sensor_id(struct subdrv_ctx *ctx)
{
	...
	SC202CSSJ_LOG_DBG("[%s] sensor_id: 0x%x", __func__, sensor_id);
...
static int get_imgsensor_id(struct subdrv_ctx *ctx, UINT32 *sensor_id)
{
	...
	unsigned char buf[50];
  	char module_id;
	UINT32 module_id_addr = EEPROM_SC202CSSJ_MOLDULEID_0_ADDR;
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor_8(ctx, 0x3107) << 8)
			...
			if (module_id != EEPROM_SC202CSSJ_MOLDULEID){
				module_id = read_cmos_sensor_8(ctx, EEPROM_SC202CSSJ_MOLDULEID_1_ADDR);
				module_id_addr = EEPROM_SC202CSSJ_MOLDULEID_1_ADDR;
			}
			LOG_INF("[sc202cssj_camera_sensor]get_imgsensor_id: ! imgsensor_info.sensor_id: 0x%x, sensor id: 0x%x, module_id = %c\n",imgsensor_info.sensor_id, *sensor_id, module_id);
			if (*sensor_id == imgsensor_info.sensor_id && module_id == EEPROM_SC202CSSJ_MOLDULEID ) {//判断是否是二供
				LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n", ctx->i2c_write_id, *sensor_id);

				for(i = 0; i < 20; i++)
				{
					if(((i != 3) && (i != 4 ) && (i != 5) && (i != 6) && (i != 7)) && module_id_addr == EEPROM_SC202CSSJ_MOLDULEID_0_ADDR )//没有烧录错误
					{
							depth_camera_sn1[i]=read_cmos_sensor_8(ctx, module_id_addr+i);
							sprintf(buf, "%02x", depth_camera_sn1[i]);
							strcat(depth_camera_sn, buf);
							LOG_INF("sc202cssj i= %d reg = 0x%x SN1: 0x%x \n",i,EEPROM_SC202CSSJ_MOLDULEID_0_ADDR+i,depth_camera_sn1[i]);
					} else if(((i != 6) && (i != 8 ) ) && module_id_addr == EEPROM_SC202CSSJ_MOLDULEID_1_ADDR )//烧录错误
					{
							depth_camera_sn1[i]=read_cmos_sensor_8(ctx, module_id_addr+i);
							sprintf(buf, "%02x", depth_camera_sn1[i]);
							strcat(depth_camera_sn, buf);
							LOG_INF("sc202cssj i= %d reg = 0x%x SN1: 0x%x \n",i,EEPROM_SC202CSSJ_MOLDULEID_1_ADDR+i,depth_camera_sn1[i]);
					}
				}
				for( i = 0; i < 20; i++){
					if( i==16 || i==17 || i==18 || i==19)
					depth_camera_sn[i] = '0';
				LOG_INF("sc202cssj  SN: 0x%x,str=%c \n",depth_camera_sn[i],depth_camera_sn[i]);
				}
				depth_camera_sn[20]='\0';

				LOG_INF("sc202cssj SNstr = %s \n",depth_camera_sn);
				main_module_id[0] = depth_camera_sn[0];
				main_module_id[1] = ',';
				LOG_INF("sc202cssj main_module_id = %s \n",main_module_id);	
				return ERROR_NONE;
			} else {
	...
	if (*sensor_id != imgsensor_info.sensor_id||
	    (module_id != EEPROM_SC202CSSJ_MOLDULEID)) {
...
static int open(struct subdrv_ctx *ctx)
{
	...
	SC202CSSJ_LOG_DBG("preview 1608*1204@30.1fps; capture1608*1204@30.1fps\n");
				  ...
				  UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_ANA_GAIN_TABLE:
		if ((void *)(uintptr_t) (*(feature_data + 1)) == NULL) {
			*(feature_data + 0) =
				sizeof(sc202cssj_ana_gain_table);
		} else {
			memcpy((void *)(uintptr_t) (*(feature_data + 1)),
			(void *)sc202cssj_ana_gain_table,
			sizeof(sc202cssj_ana_gain_table));
		}
		break;
	...
	case SENSOR_FEATURE_GET_PDAF_DATA:
		SC202CSSJ_LOG_DBG("SENSOR_FEATURE_GET_PDAF_DATA\n");
		break;
	...
	case SENSOR_FEATURE_GET_BINNING_TYPE:
		...
		SC202CSSJ_LOG_DBG("SENSOR_FEATURE_GET_BINNING_TYPE AE_binning_type:%d,\n",
...
const struct subdrv_entry sc202cssj_mipi_raw_entry = {
	.name = "sc202cssj_mipi_raw",
	.id = SC202CSSJ_SENSOR_ID,
	.pw_seq = pw_seq,
