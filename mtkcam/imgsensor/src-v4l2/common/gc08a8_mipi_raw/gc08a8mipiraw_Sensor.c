...
static int gc08a8_get_imgsensor_id(struct subdrv_ctx *ctx, u32 *para);
...
//	{SENSOR_FEATURE_CHECK_SENSOR_ID, gc08a8_get_imgsensor_id},
};
...
static struct subdrv_ops ops = {
	.get_id = gc08a8_get_imgsensor_id,
...
static int gc08a8_get_imgsensor_id(struct subdrv_ctx *ctx,u32 *para)
{
	...
	while (ctx->s_ctx.i2c_addr_table[i] != 0xFF) {
		...
		do {
			...
			LOG_INF("[gc08a8]: i2c write id: 0x%x, sensor id: 0x%x, module id: 0x%x\n",
			...
			LOG_INF("[gc08a8]: Read sensor id fail! i2c write id: 0x%x, sensor id: 0x%x\n",
...
static void gc08a8_set_max_framerate(struct subdrv_ctx *ctx, UINT16 framerate, kal_bool min_framelength_en)
{
	...
	if (shutter_cnt == 1) {
		...
		if (ctx->autoflicker_en) {
			...
			if (realtime_fps >= 593 && realtime_fps <= 607)
				gc08a8_set_max_framerate(ctx, 592, 0);
			else if (realtime_fps >= 297 && realtime_fps <= 305)
				gc08a8_set_max_framerate(ctx, 296, 0);
			else if (realtime_fps >= 147 && realtime_fps <= 150)
				gc08a8_set_max_framerate(ctx, 146, 0);
		}
