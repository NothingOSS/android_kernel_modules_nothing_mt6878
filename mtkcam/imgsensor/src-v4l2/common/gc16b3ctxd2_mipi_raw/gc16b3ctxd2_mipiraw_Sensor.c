...
#include "gc16b3ctxd2_mipiraw_Sensor.h"
#include "gc16b3ctxd2_ana_gain_table.h"
#include "gc16b3ctxd2_Sensor_setting.h"

#include "adaptor-subdrv.h"
#include "adaptor-i2c.h"
#include "adaptor.h"

#define LOG_TAG "[gc16b3ctxd2]"
...
#define gc16b3ctxd2_table_write_cmos_sensor(...) subdrv_i2c_wr_regs_u8(__VA_ARGS__)


#undef VENDOR_EDIT

static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = GC16B3CTXD2_SENSOR_ID,
...
#if GC16B3CTXD_MIRROR_NORMAL
    .sensor_output_dataformat = SENSOR_OUTPUT_FORMAT_RAW_R,
  #elif GC16B3CTXD_MIRROR_HV
				     ...
				     kal_bool auto_extend_en)
{
	...
	if (ctx->autoflicker_en) {
		...
		if (realtime_fps >= 297 && realtime_fps <= 305)
			...
			write_cmos_sensor_8(ctx, 0x0340, ctx->frame_length >> 8);
			LOG_INF("0x0340=0x%x",read_cmos_sensor_8(ctx,0x0340));
			write_cmos_sensor_8(ctx, 0x0341, ctx->frame_length & 0xFF);
			LOG_INF("0x0341=0x%x",read_cmos_sensor_8(ctx,0x0341));
		}
	} else {
			write_cmos_sensor_8(ctx, 0x0340, ctx->frame_length >> 8);
			LOG_INF("0x0340=0x%x",read_cmos_sensor_8(ctx,0x0340));
	...
	//shutter = shutter/10;
	write_cmos_sensor_8(ctx, 0x0202, shutter >> 8);
	LOG_INF("0x0202=0x%x",read_cmos_sensor_8(ctx,0x0202));
	write_cmos_sensor_8(ctx, 0x0203, shutter & 0xFF);
	LOG_INF("0x0203=0x%x",read_cmos_sensor_8(ctx,0x0203));
	LOG_INF(
...
{
	int i = 0;
	int num = sizeof(gc16b3ctxd2_init_setting)/sizeof(kal_uint16);

	for (i = 0; i < num; ){
		write_cmos_sensor_8(ctx, gc16b3ctxd2_init_setting[i], gc16b3ctxd2_init_setting[i+1]);
		i = i+2;
	}
}
...
{
	int i = 0;
	int num = sizeof(gc16b3ctxd2_2320x1744_30fps_addr_data)/sizeof(kal_uint16);

	for (i = 0; i < num; ){
		write_cmos_sensor_8(ctx, gc16b3ctxd2_2320x1744_30fps_addr_data[i], gc16b3ctxd2_2320x1744_30fps_addr_data[i+1]);
		i = i+2;
	}
}
...
{
	int i = 0;
	int num = sizeof(gc16b3ctxd2_2304x1296_30fps_addr_data)/sizeof(kal_uint16); // 修改为2304*1296

	for (i = 0; i < num; ){
		write_cmos_sensor_8(ctx, gc16b3ctxd2_2304x1296_30fps_addr_data[i], gc16b3ctxd2_2304x1296_30fps_addr_data[i+1]);
		i = i+2;
	}
}
...
{
	int i = 0;
	int num = sizeof(gc16b3ctxd2_1920x1080_60fps_addr_data)/sizeof(kal_uint16);

	for (i = 0; i < num; ){
		write_cmos_sensor_8(ctx, gc16b3ctxd2_1920x1080_60fps_addr_data[i], gc16b3ctxd2_1920x1080_60fps_addr_data[i+1]);
		i = i+2;
	}
}
...
{
	LOG_INF("Start\n");
	gc16b3ctxd2_table_write_cmos_sensor(ctx, gc16b3ctxd2_init_setting,
		sizeof(gc16b3ctxd2_init_setting)/sizeof(kal_uint16));
	LOG_INF("End\n");
}
...
static int find_cam_dis(void)
{
       ...
       // 根据路径查找节点
       my_node = of_find_node_by_name(NULL,"sensor1");
       if (!my_node) {
           printk("Failed to find the node\n");
           return -ENODEV;
       }

       // 从找到的节点中读取整数属性
       ret = of_property_read_u32(my_node, "cam_dis", &my_int_value);
       if (ret) {
           printk("Failed to read my_int_property\n");
           return ret;
       }
       pr_info("The value of my_int_property is: %u\n", my_int_value);
...
static int get_imgsensor_id(struct subdrv_ctx *ctx, UINT32 *sensor_id)
{
	...
	cam_dis = find_cam_dis();
	pr_info("cam_dis =%d",cam_dis);
	if(cam_dis == 0)
	{
		pr_info("need enter 23114 prj");
		return ERROR_SENSOR_CONNECT_FAIL;
	}
	pr_info("enter 24121 prj");
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		ctx->i2c_write_id = imgsensor_info.i2c_addr_table[i];
		do {
			*sensor_id = return_sensor_id(ctx) + 2;
			adaptor_i2c_rd_p8(ctx->i2c_client,0xa0 >> 1,
				      module_id_addr,&module_id, 1);
			LOG_INF("[gc16b3ctxd2_camera_sensor]get_imgsensor_id: ! imgsensor_info.sensor_id: 0x%x, sensor id: 0x%x\n",imgsensor_info.sensor_id, *sensor_id);
			if (*sensor_id == imgsensor_info.sensor_id && (module_id == 0x07)) {
				LOG_INF("[gc16b3ctxd2_camera_sensor]get_imgsensor_id: i2c write id: 0x%x, sensor id: 0x%x\n",
					ctx->i2c_write_id, *sensor_id);
				return ERROR_NONE;
			}
			LOG_INF("[gc16b3ctxd2_camera_sensor]get_imgsensor_id: Read sensor id fail! i2c write id: 0x%x, sensor id: 0x%x\n",
				ctx->i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
...
static int open(struct subdrv_ctx *ctx)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id(ctx) + 2;
			if (sensor_id == imgsensor_info.sensor_id) {
				LOG_INF("[gc16b3ctxd2_camera_sensor]open: i2c write id: 0x%x, sensor id: 0x%x\n",
					ctx->i2c_write_id, sensor_id);
				break;
			}
			LOG_INF("[gc16b3ctxd2_camera_sensor]open: Read sensor id fail, write id: 0x%x, id: 0x%x\n",
				ctx->i2c_write_id, sensor_id);
			retry--;
		} while (retry > 0);
		i++;
		...
		UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_ANA_GAIN_TABLE:
		if ((void *)(uintptr_t) (*(feature_data + 1)) == NULL) {
			*(feature_data + 0) =
				sizeof(gc16b3ctxd2_ana_gain_table);
		} else {
			memcpy((void *)(uintptr_t) (*(feature_data + 1)),
			(void *)gc16b3ctxd2_ana_gain_table,
			sizeof(gc16b3ctxd2_ana_gain_table));
		}
		break;
...
const struct subdrv_entry gc16b3ctxd2_mipi_raw_entry = {
	.name = "gc16b3ctxd2_mipi_raw",
	.id = GC16B3CTXD2_SENSOR_ID,
	.pw_seq = pw_seq,
