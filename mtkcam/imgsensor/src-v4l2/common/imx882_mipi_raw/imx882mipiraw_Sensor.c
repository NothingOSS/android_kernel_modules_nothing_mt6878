...
#include "imx882_ana_gain_table.h"
...
#define imx882_table_write_cmos_sensor(...) subdrv_i2c_wr_regs_u8(__VA_ARGS__)
#undef VENDOR_EDIT
...
static void write_sensor_QSC(struct subdrv_ctx *ctx)
{
	...
	{
		imx882_table_write_cmos_sensor(ctx, imx882_QSC_setting,
...
//{
/*	u8 imx882_LRC_data[352] = {0};
	LOG_DEBUG("E");
	read_imx882_LRC(ctx, imx882_LRC_data);
	subdrv_i2c_wr_p8(ctx, 0x7520, imx882_LRC_data, 70);
	subdrv_i2c_wr_p8(ctx, 0x7568, imx882_LRC_data + 70, 70);
...
static kal_uint16 imx882_seamless_preview[] = {
	 0x0112, 0x0A,
        0x0113, 0x0A,
        0x0114, 0x02,
//Line Length PCK Setting
...
static kal_uint16 imx882_seamless_custom3[] = {
    0x0112,0x0A,
    0x0113,0x0A,
    0x0114,0x02,
    0x0342,0x23,
...
{
	LOG_DEBUG("E\n");
	imx882_table_write_cmos_sensor(ctx, imx882_init_setting,
...
{
	LOG_DEBUG("E\n");
	imx882_table_write_cmos_sensor(ctx, imx882_preview_setting,
...
static void capture_setting(struct subdrv_ctx *ctx, kal_uint16 currefps)
{
	...
	/*************MIPI output setting************/
	imx882_table_write_cmos_sensor(ctx, imx882_capture_30_setting,
...
static void normal_video_setting(struct subdrv_ctx *ctx, kal_uint16 currefps)
{
	...
	imx882_table_write_cmos_sensor(ctx, imx882_normal_video_setting,
...
static void hs_video_setting(struct subdrv_ctx *ctx)
{
	...
	/*************MIPI output setting************/
	imx882_table_write_cmos_sensor(ctx, imx882_hs_video_setting,
...
{
	LOG_DEBUG("%s E\n", __func__);
	imx882_table_write_cmos_sensor(ctx, imx882_slim_video_setting,
...
static void custom1_setting(struct subdrv_ctx *ctx)
{
	...
	/*************MIPI output setting************/
	imx882_table_write_cmos_sensor(ctx, imx882_custom1_setting,
...
static void custom2_setting(struct subdrv_ctx *ctx)
{
	...
	/*************MIPI output setting************/
	imx882_table_write_cmos_sensor(ctx, imx882_custom2_setting,
...
static void custom3_setting(struct subdrv_ctx *ctx)
{
	...
	/*************MIPI output setting************/
	imx882_table_write_cmos_sensor(ctx, imx882_custom3_setting,
...
static int get_imgsensor_id(struct subdrv_ctx *ctx, UINT32 *sensor_id)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor_8(ctx, 0x0016) << 8)
			...
			LOG_INF("[imx882_camera_sensor]get_imgsensor_id: ! imgsensor_info.sensor_id: 0x%x, sensor id: 0x%x, module_id = %c\n",imgsensor_info.sensor_id, *sensor_id, module_id);
			if (*sensor_id == imgsensor_info.sensor_id && (module_id == 0x01)) {
				LOG_INF("[imx882_camera_sensor]get_imgsensor_id: i2c write id: 0x%x, sensor id: 0x%x\n",
					ctx->i2c_write_id, *sensor_id);
				return ERROR_NONE;
			}
			LOG_INF("[imx882_camera_sensor]get_imgsensor_id: Read sensor id fail! i2c write id: 0x%x, sensor id: 0x%x\n",
				ctx->i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
	...
	if (*sensor_id != imgsensor_info.sensor_id ||
		 ...
		 */
		*sensor_id = 0x0;
		LOG_INF("[imx882_camera_sensor]get_imgsensor_id: sensor id: 0x%x\n", *sensor_id);
...
static kal_uint32 seamless_switch(struct subdrv_ctx *ctx,enum MSDK_SCENARIO_ID_ENUM scenario_id, struct mtk_hdr_ae *ae_ctrl)
{
    ...
    case SENSOR_SCENARIO_ID_CUSTOM3:
    {
        ...
        imx882_table_write_cmos_sensor(ctx ,imx882_seamless_custom3, sizeof(imx882_seamless_custom3) / sizeof(kal_uint16));
        if (ae_ctrl) {
    ...
    case SENSOR_SCENARIO_ID_NORMAL_PREVIEW:
    {
        ...
        write_cmos_sensor_8(ctx, 0x0104, 0x01);
        write_cmos_sensor_8(ctx, 0x3010, 0x02);
        imx882_table_write_cmos_sensor(ctx ,imx882_seamless_preview, sizeof(imx882_seamless_preview) / sizeof(kal_uint16));
        if (ae_ctrl) {
...
static void feedback_awbgain(struct subdrv_ctx *ctx, kal_uint32 r_gain, kal_uint32 b_gain)
{
	...
	if (ctx->sensor_mode == IMGSENSOR_MODE_CUSTOM3) {
		...
		imx882_feedback_awbgain[7] = r_gain_int;
		imx882_feedback_awbgain[9] = (r_gain - r_gain_int * 512) / 2;
		imx882_feedback_awbgain[11] = b_gain_int;
		imx882_feedback_awbgain[13] = (b_gain - b_gain_int * 512) / 2;
		imx882_table_write_cmos_sensor(ctx ,imx882_feedback_awbgain,
			sizeof(imx882_feedback_awbgain)/sizeof(kal_uint16));
	}
}
static int imx882main_set_awb_gain(struct subdrv_ctx *ctx, u8 *para) {
				 ...
				 UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_ANA_GAIN_TABLE:
		...
		case SENSOR_SCENARIO_ID_CUSTOM3:
			if ((void *)(uintptr_t) (*(feature_data + 1)) == NULL) {
				*(feature_data + 0) =
					sizeof(imx882_ana_0db_gain_table);
			} else {
				memcpy((void *)(uintptr_t) (*(feature_data + 1)),
				(void *)imx882_ana_0db_gain_table,
				sizeof(imx882_ana_0db_gain_table));
			}
		break;
		default :
		if ((void *)(uintptr_t) (*(feature_data + 1)) == NULL) {
				*(feature_data + 0) =
					sizeof(imx882_ana_64db_gain_table);
			} else {
				memcpy((void *)(uintptr_t) (*(feature_data + 1)),
				(void *)imx882_ana_64db_gain_table,
				sizeof(imx882_ana_64db_gain_table));
			}
		break;
		}
    ...
    case SENSOR_FEATURE_SET_AWB_GAIN:
        ...
        imx882main_set_awb_gain(ctx, (u8 *)(feature_data));
	break;
...
const struct subdrv_entry imx882_mipi_raw_entry = {
	.name = "imx882_mipi_raw",
	.id = IMX882_SENSOR_ID,
	.pw_seq = pw_seq,
