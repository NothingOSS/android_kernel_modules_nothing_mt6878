...
#include "gc16b3cxl_mipiraw_Sensor.h"
#include "gc16b3cxl_ana_gain_table.h"
#include "gc16b3cxl_Sensor_setting.h"

#include "adaptor-subdrv.h"
#include "adaptor-i2c.h"
#include "adaptor.h"

#define LOG_TAG "[gc16b3cxl]"
...
#define gc16b3cxl_table_write_cmos_sensor(...) subdrv_i2c_wr_regs_u8(__VA_ARGS__)


#undef VENDOR_EDIT

static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = GC16B3CXL_SENSOR_ID,
...
#if GC16B3CXL_MIRROR_NORMAL
    .sensor_output_dataformat = SENSOR_OUTPUT_FORMAT_RAW_R,
  #elif GC16B3CXL_MIRROR_HV
...
{
	int i = 0;
	int num = sizeof(gc16b3cxl_init_setting)/sizeof(kal_uint16);

	for (i = 0; i < num; ){
		write_cmos_sensor_8(ctx, gc16b3cxl_init_setting[i], gc16b3cxl_init_setting[i+1]);
		i = i+2;
	}
}
...
{
	int i = 0;
	int num = sizeof(gc16b3cxl_2320x1744_30fps_addr_data)/sizeof(kal_uint16);

	for (i = 0; i < num; ){
		write_cmos_sensor_8(ctx, gc16b3cxl_2320x1744_30fps_addr_data[i], gc16b3cxl_2320x1744_30fps_addr_data[i+1]);
		i = i+2;
	}
}
...
{
	int i = 0;
	int num = sizeof(gc16b3cxl_2304x1296_30fps_addr_data)/sizeof(kal_uint16); // 修改为2304*1296

	for (i = 0; i < num; ){
		write_cmos_sensor_8(ctx, gc16b3cxl_2304x1296_30fps_addr_data[i], gc16b3cxl_2304x1296_30fps_addr_data[i+1]);
		i = i+2;
	}
}
...
{
	int i = 0;
	int num = sizeof(gc16b3cxl_1920x1080_60fps_addr_data)/sizeof(kal_uint16);

	for (i = 0; i < num; ){
		write_cmos_sensor_8(ctx, gc16b3cxl_1920x1080_60fps_addr_data[i], gc16b3cxl_1920x1080_60fps_addr_data[i+1]);
		i = i+2;
	}
}
...
{
	LOG_INF("Start\n");
	gc16b3cxl_table_write_cmos_sensor(ctx, gc16b3cxl_init_setting,
		sizeof(gc16b3cxl_init_setting)/sizeof(kal_uint16));
	LOG_INF("End\n");
}
...
static int get_imgsensor_id(struct subdrv_ctx *ctx, UINT32 *sensor_id)
{
	...
	if(cam_dis == 0)
	{
		pr_info("need enter prj");
		return ERROR_SENSOR_CONNECT_FAIL;
	}
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		ctx->i2c_write_id = imgsensor_info.i2c_addr_table[i];
		do {
			*sensor_id = return_sensor_id(ctx) + 4;
			adaptor_i2c_rd_p8(ctx->i2c_client,0xa0 >> 1,
				      module_id_addr,&module_id, 1);

			LOG_INF("[gc16b3cxl]sensor_id: 0x%x, sensor id: 0x%x, module id: 0x%x\n",imgsensor_info.sensor_id, *sensor_id, module_id);
			if (*sensor_id == imgsensor_info.sensor_id && (module_id == 0x09)) {
				LOG_INF("[gc16b3cxl]i2c write id: 0x%x, sensor id: 0x%x\n",
					ctx->i2c_write_id, *sensor_id);
				return ERROR_NONE;
			}
			LOG_INF("[gc16b3cxl]Read sensor id fail! i2c write id: 0x%x, sensor id: 0x%x\n",
				ctx->i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
...
static int open(struct subdrv_ctx *ctx)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id(ctx) + 4;
			if (sensor_id == imgsensor_info.sensor_id) {
				LOG_INF("[gc16b3cxl_camera_sensor]open: i2c write id: 0x%x, sensor id: 0x%x\n",
					ctx->i2c_write_id, sensor_id);
				break;
			}
			LOG_INF("[gc16b3cxl_camera_sensor]open: Read sensor id fail, write id: 0x%x, id: 0x%x\n",
				ctx->i2c_write_id, sensor_id);
			retry--;
		} while (retry > 0);
		i++;
		...
		UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_ANA_GAIN_TABLE:
		if ((void *)(uintptr_t) (*(feature_data + 1)) == NULL) {
			*(feature_data + 0) =
				sizeof(gc16b3cxl_ana_gain_table);
		} else {
			memcpy((void *)(uintptr_t) (*(feature_data + 1)),
			(void *)gc16b3cxl_ana_gain_table,
			sizeof(gc16b3cxl_ana_gain_table));
		}
		break;
...
const struct subdrv_entry gc16b3cxl_mipi_raw_entry = {
	.name = "gc16b3cxl_mipi_raw",
	.id = GC16B3CXL_SENSOR_ID,
	.pw_seq = pw_seq,
