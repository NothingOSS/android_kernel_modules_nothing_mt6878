 ...
 */
 ...
 extern char micro_camera_sn[64];

 unsigned int do_2a_gain_gc02m1(struct EEPROM_DRV_FD_DATA *pdata,
		 unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
 unsigned int do_part_number_gc02m1(struct EEPROM_DRV_FD_DATA *pdata,
		 unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
 ...
 static struct STRUCT_CALIBRATION_LAYOUT_STRUCT cal_layout_table = {
	 0x00000001, 0x010b00ff, CAM_CAL_SINGLE_EEPROM_DATA,
	 {
		 ...
		 {0x00000001, 0x00000008, 0x00000014, do_part_number_gc02m1},
		 {0x00000001, 0x00000030, 0x0000074C, do_single_lsc},
		 {0x00000001, 0x0000001E, 0x00000010, do_2a_gain_gc02m1},
 ...
 struct STRUCT_CAM_CAL_CONFIG_STRUCT gc02m1_mtk_eeprom = {
	 .name = "gc02m1_mtk_eeprom",
		 ...
		 unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
 {
	 ...
	 if (read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
		 ...
		 must_log("GC02M1_SENSOR_ID  == pCamCalData->sensorIDmodule ID = 0x%x,0x01 from SXL,0x09 from XL \n",pCamCalData->PartNumber[0]);
		 must_log("main_module_id ID = %s,buf=%s \n",main_module_id,buf);
		 main_module_id[0]=buf[0];
		 main_module_id[1]= buf[1];
		 main_module_id[2]= ',';
		 must_log("main_module_id ID = %s,buf=%s \n",main_module_id,buf);
		 }
	 else {
	 ...
	 sprintf(micro_camera_sn, "%s", pCamCalData->PartNumber);
