 ...
 */
 ...
 unsigned int do_2a_gain_gc08a8syx(struct EEPROM_DRV_FD_DATA *pdata,
		 unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
 unsigned int do_part_number_gc08a8syx(struct EEPROM_DRV_FD_DATA *pdata,
		 unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
 ...
 static struct STRUCT_CALIBRATION_LAYOUT_STRUCT cal_layout_table = {
	 0x00000001, 0x010b00ff, CAM_CAL_SINGLE_EEPROM_DATA,
	 {
		 ...
		 {0x00000001, 0x00000008, 0x00000014, do_part_number_gc08a8syx},
		 {0x00000001, 0x00000044, 0x0000074C, do_single_lsc},
		 {0x00000001, 0x0000001E, 0x00000010, do_2a_gain_gc08a8syx},
 ...
 struct STRUCT_CAM_CAL_CONFIG_STRUCT gc08a8syx_mtk_eeprom = {
	 .name = "gc08a8syx_mtk_eeprom",
		 ...
		 unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
 {
	 ...
	 if (read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
		 ...
		 must_log("GC08A8SYX_SENSOR_ID  == pCamCalData->sensorIDmodule ID = 0x%x,0x01 from SXL,0x09 from XL \n",pCamCalData->PartNumber[0]);
		 must_log("main_module_id ID = %s,buf=%s \n",main_module_id,buf);
		 main_module_id[7]=buf[0];
		 main_module_id[8]= ',';
		 must_log("main_module_id ID = %s,buf=%s \n",main_module_id,buf);
		 }
	 else {
