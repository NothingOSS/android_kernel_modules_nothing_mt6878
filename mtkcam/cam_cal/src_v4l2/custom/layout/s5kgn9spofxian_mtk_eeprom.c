 ...
 */
 ...
 unsigned int do_2a_gain_s5kgn9ofxian(struct EEPROM_DRV_FD_DATA *pdata,
		 unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
 unsigned int s5kn9spofxian_do_part_number(struct EEPROM_DRV_FD_DATA *pdata,
			unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
 static struct STRUCT_CALIBRATION_LAYOUT_STRUCT cal_layout_table = {
	 0x00000001, 0x010b00ff, CAM_CAL_SINGLE_EEPROM_DATA,
	 {
		 ...
		 {0x00000001, 0x00000008, 0x00000014, s5kn9spofxian_do_part_number},
		 {0x00000001, 0x00000044, 0x0000074C, do_single_lsc},
		 {0x00000001, 0x00000026, 0x00000017, do_2a_gain_s5kgn9ofxian},
 ...
 struct STRUCT_CAM_CAL_CONFIG_STRUCT s5kgn9spofxian_mtk_eeprom = {
	 .name = "s5kgn9spofxian_mtk_eeprom",
	 .check_layout_function = layout_check,
	 .read_function = Common_read_region,
	 .layout = &cal_layout_table,
	 .sensor_id = S5KGN9SPOFXIAN_SENSOR_ID,
	 .i2c_write_id = 0xA0,
	 .max_size = 0x4000,
	 .enable_preload = 1,
	 .preload_size = 0x1500,
	...
	unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	if (read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
		...
		must_log("s5kn9spofxian module ID = 0x%x,0X07 from TXD,0x01 from SYX \n",pCamCalData->PartNumber[0]);
		must_log("main_module_id ID = %s,buf=%s \n",main_module_id,buf);
		main_module_id[5]=buf[0];
		main_module_id[6]= ',';
		must_log("main_module_id ID = %s,buf=%s \n",main_module_id,buf);
		}
	else {
