...
unsigned int do_2a_gain_gc16b3cxl(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
unsigned int gc16bcxl_do_part_number(struct EEPROM_DRV_FD_DATA *pdata,
			unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static struct STRUCT_CALIBRATION_LAYOUT_STRUCT cal_layout_table = {
	0x0001, 0x001, CAM_CAL_SINGLE_EEPROM_DATA,
	{
		...
		{0x00000001, 0x00000008, 0x00000014, gc16bcxl_do_part_number},
		{0x00000001, 0x00000030, 0x0000074C, do_single_lsc},
		{0x00000001, 0x0000001E, 0x0000000E, do_2a_gain_gc16b3cxl},
...
struct STRUCT_CAM_CAL_CONFIG_STRUCT gc16b3cxl_mtk_eeprom = {
	.name = "gc16b3cxl_mtk_eeprom",
	...
	unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	if (read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
		...
		must_log("gc16bcxl gc16bcxl  == pCamCalData->sensorIDmodule ID = 0x%x,0x09 from XL,0x0A from OFILM \n",pCamCalData->PartNumber[0]);
		must_log("main_module_id ID = %s,buf=%s \n",main_module_id,buf);
		main_module_id[3]=buf[0];
		main_module_id[4]= ',';
		must_log("main_module_id ID = %s,buf=%s \n",main_module_id,buf);
		}
	else {
