...
unsigned int do_2a_gain_ov50d40ofilm(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);

unsigned int do_part_number_ov50d40ofilm(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
...
static struct STRUCT_CALIBRATION_LAYOUT_STRUCT cal_layout_table = {
	0x00000001, 0x010b00ff, CAM_CAL_SINGLE_EEPROM_DATA,
	{
		...
		{0x00000001, 0x00000008, 0x00000014, do_part_number_ov50d40ofilm},
		{0x00000001, 0x00000030, 0x0000074C, do_single_lsc},
		{0x00000001, 0x0000001E, 0x00000010, do_2a_gain_ov50d40ofilm},
		{0x00000001, 0x00000950, 0x000005de, do_pdaf},
...
struct STRUCT_CAM_CAL_CONFIG_STRUCT ov50d40ofilm_mtk_eeprom = {
	.name = "ov50d40ofilm_mtk_eeprom",
		...
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	if (AFConfig) {
		...
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
				addr_AF+2, 2, (unsigned char *)&AFMacro);
		...
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	if (read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
		...
		must_log("OV50D40OFILM_SENSOR_ID  == pCamCalData->sensorIDmodule ID = 0x%x,0x09 from XL,0x0A from OFILM \n",pCamCalData->PartNumber[0]);
