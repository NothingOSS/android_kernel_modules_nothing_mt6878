		...
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	if(IMX882_SENSOR_ID  == pCamCalData->sensorID || IMX882TXD_SENSOR_ID  == pCamCalData->sensorID || S5KGN9SP_SENSOR_ID == pCamCalData->sensorID){
		...
		if (read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			...
			memset(buf, 0, sizeof(buf));
			sprintf(buf, "%x", pCamCalData->PartNumber[0]);
			must_log("GC16B3C_SENSOR_ID GC16B3CTXD_SENSOR_ID  == pCamCalData->sensorIDmodule ID = 0x%x,0X07 from TXD,0x01 from SYX \n",pCamCalData->PartNumber[0]);
			must_log("main_module_id ID = %s,buf=%s \n",main_module_id,buf);
		...
		if (read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			...
			memset(buf, 0, sizeof(buf));
			sprintf(buf, "%x", pCamCalData->PartNumber[0]);
			must_log("GC08A8_SENSOR_ID  == pCamCalData->sensorIDmodule ID = 0x%x,0X07 from TXD,0x01 from SYX \n",pCamCalData->PartNumber[0]);
			must_log("main_module_id ID = %s,buf=%s \n",main_module_id,buf);
		...
		if (read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			...
			memset(buf, 0, sizeof(buf));
			sprintf(buf, "%x", pCamCalData->PartNumber[0]);
			must_log("ov50d40_SENSOR_ID  == pCamCalData->sensorIDmodule ID = 0x%x,0X07 from TXD,0x01 from SYX \n",pCamCalData->PartNumber[0]);
			must_log("main_module_id ID = %s,buf=%s \n",main_module_id,buf);
